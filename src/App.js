...
export default function App() {
  ...
  const transplantToPond = (pondIndex) => {
    ...
    let oldestReadyday = 0;
    let oldestReady;
    for (const g of germinationCopy.trays) {
      if (day - g.ready >= oldestReadyday) {
        oldestReady = g;
      }
    }
    if (oldestReady !== undefined) {
      ...
      const plantsAvailable = oldestReady.plants;
      const plantsUsed = Math.min(plantsRequested, plantsAvailable);

      oldestReady.plants -= plantsUsed;
  ...
  /**
   * Harvests seedlings from the oldest raft
   * @param {number} pondIndex the index of the pond to transplant to
   */

  const harvestPlant = (pondIndex) => {
    const germinationCopy = _.cloneDeep(germination);
    const productionCopy = _.cloneDeep(production);

    const harvestedRaft = productionCopy[pondIndex].rafts.pop();

    const plantsHarvested = harvestedRaft.plants;

    if (plantsHarvested) {
      setHarvested(harvested + plantsHarvested);
    }
    setGermination(germinationCopy);
    setProduction(productionCopy);
  };
  ...
  return (
    <div className="App">
      ...
      <h3 className="fs-3">Harvested: {harvested}</h3>
          ...
          <div className="row mx-2 mb-3 g-0 align-items-center" key={pond.name}>
            ...
            <div className="col-1">
              <button
                className="btn btn-light position-relative"
                style={{}}
                onClick={() => harvestPlant(pondIndex)}
                disabled={!germination.trays.length}
              >
                Harvest
              </button>
